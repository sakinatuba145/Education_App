...
class _TeacherCourseHubScreenState extends State<TeacherCourseHubScreen>
  ...
  Widget _buildCertificatesTab() {
    return StreamBuilder<QuerySnapshot>(
      stream: _db
          ...
          .where('projectPassed', isEqualTo: true)
          .snapshots(),
  ...
  Widget _certStudentCard(Map<String, dynamic> data, String studentId) {
    final status = data['status'] as String? ?? 'completed';
    final progress = ((data['progress'] as num?)?.toDouble() ?? 1.0) * 100;
    final score = data['projectScore'] as int? ?? 0;
    final name = data['studentName'] as String?;

    return FutureBuilder<DocumentSnapshot>(
      future: _db.collection('users').doc(studentId).get(),
      builder: (ctx, userSnap) {
        String displayName = name ?? studentId;
        String email = '';
        if (userSnap.hasData && userSnap.data!.exists) {
          final ud = userSnap.data!.data() as Map<String, dynamic>;
          final raw = (ud['displayName'] as String?) ?? '';
          displayName = raw.contains('|')
              ? raw.split('|').first
              : (raw.isNotEmpty ? raw : displayName);
